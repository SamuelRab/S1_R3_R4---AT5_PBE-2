...
const pedidoRepository = {
    ...
    selecionar: async () => {
        const sql = `
            SELECT p.id, p.dataPedido, p.valorTotal, p.statusPedido,
                   ip.id AS itemId, ip.idProduto, pr.nome AS nomeProduto,
                   ip.quantidade, ip.valorUnitario, ip.subtotal
            FROM pedidos p
            LEFT JOIN itens_pedido ip ON ip.idPedido = p.id
            LEFT JOIN produtos pr ON pr.id = ip.idProduto
            ORDER BY p.id DESC
        `;
    ...
    recalcularValorTotal: async (conn, idPedido) => {
        ...
        const novoTotal = itens.reduce((total, item) => total + item.valorUnitario * item.quantidade, 0);
